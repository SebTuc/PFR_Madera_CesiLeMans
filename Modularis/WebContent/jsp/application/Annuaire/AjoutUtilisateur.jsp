...
<form method="post">
  <div class="form-row">
	  ...
	  <div class="form-group col-md-6">
	    ...
	    <input type="text" class="form-control" name="prenom" id="prenom" placeholder="Prenom Utilisateur" required>
	  </div>
  </div>
  <div class="form-row">
	  <div class="form-group col-md-6">
  ...
  <div class="form-group">
   ...
   <div class="form-group ">
		...
		<select id="metier" name="metier" class="custom-select" required>
			<c:forEach var="Metier" items="${ListMetier }">
		   		<option value="${fn:escapeXml(Metier.metierId) }">${fn:escapeXml(Metier.nom) } </option>
		   	</c:forEach>
		</select>
  </div>
  <div class="form-group">
